...
def valid_neigbours(coord, max_lines, max_col):
    ...
    return list(filter(valid, neighs))
...
def flow_vertical_only(open_state):
    ...
    for i, row in enumerate(open_state[1:]):
        prev = res[i]
        res += [list(starmap(and_, zip(prev, row)))]
    return res
...
def flow_hash(open_state, direct_only=False, triangular_grid=False):
    ...
    def any_true_neigh(coord):
        ...
        elif triangular_grid is True:
            neighs = neighbours(coord, points=TRIANGULAR_GRID_POINTS)
        else:
            neighs = neighbours(coord)
        for neigh in neighs:
            if get(neigh) is True:
                return True
        return None

    def nb_false_neighbours(coord):
        return sum([get(neigh) is False for neigh in neighbours(coord)])
    ...
    def calc_for(coord):
        i, j = coord
        if i == 0:
            _set(coord, open_state[i][j])
            return
        if open_state[i][j] is False:
            _set(coord, False)
            return
        if any_true_neigh(coord) is True:
            _set(coord, True)
            return
    ...
    for i, (count, type_) in enumerate(count_types):
        if nb_true_neighbours[i] > 0:
            if count_types[i][1] is None:
                # now  count_types is no more a true rle... as it can have (2,True) followed by (1, True ) instead of (3, True)
                count_types[i] = (count, True)
...
def get_shapes_for(matr):
    ...
    for i, row in enumerate(flw):
        for j, cell in enumerate(row):
            (x, y) = j, n - i - 1
            if cell == False:
                lst_squares += [patches.Rectangle((x, y),
                                                  1, 1, facecolor='black')]
            if flw[i][j] == True:
                lst_squares += [patches.Rectangle((x, y),
                                                  1, 1, facecolor='blue')]
...
def main():
    parser = argparse.ArgumentParser(
        description='Percolation for a random matrix')
    ...
    parser.add_argument('--nb-interval-prob', type=int,
                        help='number of interval for the probabilities')
    ...
    parser.add_argument(
        '--adaptive', help='EXPERIMENTAL:feature make the intervals based on xaxis rather than the yaxis', action='store_true')
    parser.add_argument('--ascii-with-prob', type=float,
                        help='given the prob draw ascii with the given prob')
    group.add_argument('--directed', help=' ', action='store_true')
    group.add_argument(
        '--triangular-grid', help='if the grid is triangular (6 neighbours)', action='store_true')
    group.add_argument('--vertical-only', help='help for ',
                       action='store_true')
    parser.add_argument(
        '--bond', help='edges of th grid provides connectivity', action='store_true')
    ...
    if args.vertical_only:
        ...
        for i, p in enumerate(probs):
            ...
            print(success, nb_trials, success / nb_trials,
                  theoritical_vertical_percolation)
        sys.exit(0)
    if args.bond:
        n += 1
        m += 1
    ...
    def perc_func(p):
        success = experiment(n, m, p, nb_trials,
